...
module IssuesHelper
  ...
  def render_descendants_tree(issue,options={})
    ...
    issue_list(issue.descendants.visible.preload(:status, :priority, :tracker).sort_by(&:lft)) do |child, level|
      ...
      s << content_tag('tr',
             ...
             content_tag('td', link_to_issue(child, :project => (issue.project_id != child.project_id), :display_desc_in_tree => options[:display_desc_in_tree]), :class => 'subject', :style => 'width: 50%') +
